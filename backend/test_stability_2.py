"""
test_stability_2.py — Gurukul TTS Stability Test Suite

Tests the Vaani voice engine under production-like conditions to verify
runtime stability. Generates a STABILITY_TEST_LOG_2.md report.
...
Usage:
  python test_stability_2.py              # Run all tests against live server
  python test_stability_2.py --dry-run    # Syntax check only (no server needed)
...
def test_gpu_unavailability():
    ...
    # Check current GPU status from metrics endpoint
    try:
        ...
        gpu_details = system_data.get("gpu", {})
        gpu_status = "available" if gpu_details.get("available") else "CPU fallback"
        voice_engine = system_data.get("vaani", {})

        print(f"    GPU status: {gpu_status}")
        print(f"    GPU available: {gpu_details.get('available', 'unknown')}")
        print(f"    GPU device: {gpu_details.get('device_name', 'N/A')}")
        print(f"    Voice engine GPU mode: {'GPU' if gpu_details.get('available') else 'CPU fallback'}")
...
# ──────────────────────────────────────────────────────────────
def generate_report():
    """Generate STABILITY_TEST_LOG_2.md from collected results."""
    report_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "STABILITY_TEST_LOG_2.md")
    ...
    lines.append("")
    lines.append("---")
    lines.append("*Generated by `test_stability_2.py` — Gurukul TTS Stability Suite*")
