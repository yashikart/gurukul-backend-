...
const ParentsManagement = () => {
  ...
  const handleFileUpload = async (e) => {
    ...
    try {
      const result = await schoolAdminAPI.uploadParentsExcel(file);
      setUploadResult(result);
      fetchParents();
      if (result.failed_count > 0) {
        alert(`Upload completed with ${result.failed_count} failures. Check details below.`);
      } else {
        alert('All parents uploaded successfully!');
      }
    } catch (err) {
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-3xl font-bold text-gray-800">Parents Management</h1>
          <p className="text-gray-600 mt-2">Manage all parents in your school</p>
        </div>
      ...
      {showUploadForm && (
        <div className="bg-white rounded-lg shadow-md p-6">
          <h2 className="text-xl font-bold text-gray-800 mb-4">Upload Parents from Excel</h2>
          <p className="text-sm text-gray-600 mb-4">
            Excel file should have columns: <strong>name</strong>, <strong>email</strong>, <strong>student_email</strong> (optional)
          </p>
